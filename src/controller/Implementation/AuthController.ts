...
import type { IAuthService } from "../../services/Interfaces/IAuthServices";
import {
	loginSchema,
	resetPassword,
	signupSchema,
...
export class AuthController implements IAuthController {
	...
	async verifyOtp(req: Request, res: Response ,next: NextFunction): Promise<Response> {
		try {
         ...
         next(error)
			...
			return res.status(HttpStatus.BAD_REQUEST).json({
				success: false,
				message: MESSAGES.ERRORS.VALIDATION_ERROR,
			});
		}
	}
	...
	async verifyOtpFor(req: Request, res: Response ,next: NextFunction) {
		try {
         ...
         next(error)
			return res.status(HttpStatus.BAD_REQUEST).json({
				success: false,
				message: MESSAGES.AUTH.OTP_EMAIL_REQUIRED,
			});
		}
	}
	...
	async resendOtp(req: Request, res: Response ,_next: NextFunction): Promise<Response> {
		try {
			const { email } = verifyEmailSchema.parse(req.body);
	...
	async adminLogout(_req: Request, res: Response ,next: NextFunction): Promise<Response> {
		try {
			const isProduction = process.env.NODE_ENV === "production";
	...
	async googleAuth(req: Request, res: Response ,next: NextFunction): Promise<Response> {
		try {
	...
	async exampleRoute(req: Request, res: Response ,next: NextFunction): Promise<Response> {
		try {
